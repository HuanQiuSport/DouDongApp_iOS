    ...
    if (!_colors) {
        ...
        for (NSInteger deg = 0; deg <= 360; deg += 5) {
            if (self.progressType == WGradientProgressType_colorNormal) {
              [_colors addObject:(id)[UIColor colorWithPatternImage:[UIImage imageResize:KIMG(@"gradualColor") andResizeTo:self.frame.size]].CGColor];
            }else{
...
//        _gradLayer.colors = [NSArray arrayWithArray:self.colors];
        ...
        UIColor *color  = [UIColor colorWithRed: 251.0/255.0 green:65/255.0 blue:92.0/255.0 alpha:1];
        UIColor *color2  = [UIColor colorWithRed: 251.0/255.0 green:120/255.0 blue:92.0/255.0 alpha:1];
        _gradLayer.colors = @[(id)color.CGColor,(id)color2.CGColor];
...
//=======
#warning 颜色改了，但是进度条还是显示不对
        _progressColor = [UIColor colorWithPatternImage:[UIImage imageResize:KIMG(@"gradualColor") andResizeTo:self.frame.size]];;
//>>>>>>> 1.修改上传动画 2.修改录制UI界面
