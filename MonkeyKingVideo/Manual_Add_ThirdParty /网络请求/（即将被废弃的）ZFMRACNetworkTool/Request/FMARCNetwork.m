        ...
        task = [self.manager
            ...
            if (!error) {//网络OK
                ...
                if (statusCode == HTTPResponseCodeSuccess) {
                    ...
                    //                    DLog(@"网络请求参数%@",response.reqResult);
                }else{
                    /**
                     *  用户尚未登录 401
                     *  后台业务代码参数异常 参数异常 550
                     *  后台代码异常 999
                     */
#pragma mark - 用户尚未登录 401 后台业务代码参数异常 参数异常 550 后台代码异常 999
                    ...
                    
                    {
                        ...
                        if ( statusCode == HTTPResponseCodeFailureToken) {
//                            [NSObject Login];

                            SPAlertController *alert = [SPAlertController alertControllerWithTitle:@"" message:@"您在另外一台设备登录了账号，本设备的账号被强制下线，如果不是您本人操作，请尽快找回账号。" preferredStyle:SPAlertControllerStyleAlert];
                            alert.messageColor = kBlackColor;
                            alert.textAlignment = NSTextAlignmentLeft;
                            SPAlertAction *action1 = [SPAlertAction actionWithTitle:@"我知道了" style:SPAlertActionStyleDefault handler:^(SPAlertAction * _Nonnull action) {
                                 [NSObject Login];
                            }];
                            action1.titleColor = kRedColor;
                        ...
                        FMHttpResonse *response = [[FMHttpResonse alloc] initWithResponseSuccess:responseObject[HTTPServiceResponseMsgKey]
                                                                                            code:statusCode];
                        NSLog(@"异常接口路径 %@  %@ %ld",request.URL.absoluteString,responseObject[HTTPServiceResponseMsgKey],statusCode);
                        [subscriber sendNext:response];
                        [subscriber sendCompleted];
                    }else{//抛其他异常
#pragma mark - 抛其他异常
                ...
                NSLog(@"异常接口路径 %@  %@ %ld",request.URL.absoluteString,responseObject[HTTPServiceResponseMsgKey]);
...
- (RACSignal *)UploadRequestWithPath:(NSString *)path
    ...
    RACSignal *signal = [RACSignal createSignal:^(id<RACSubscriber> subscriber) {
        @strongify(self);
        NSError *serializationError = nil;
        NSString *url = [[URL_Manager sharedInstance].BaseUrl_1 stringByAppendingString:path];
...
#pragma clang diagnostic pop
        ...
        __block NSURLSessionDataTask *task = [self.manager uploadTaskWithStreamedRequest:request
            ...
            CGFloat totalsize = [parameters[@"videoSize"] floatValue] /(1024.0*1024.0);
