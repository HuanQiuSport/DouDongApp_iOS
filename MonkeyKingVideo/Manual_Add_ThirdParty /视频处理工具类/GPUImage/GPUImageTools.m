...
@property(nonatomic,strong)UIImage *imageEnd;
...
#pragma mark ——实况视频
-(void)LIVE{
    self.position = AVCaptureDevicePositionBack;//使用前置还是后置摄像头
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW,
        ...
        if ([FileFolderHandleTool createFolderByUrl:d error:nil]) {
            if (self.urlArray.count > 1) {
                [self.urlArray removeFirstObject];
            }
            ...
            NSDictionary *userInfo = @{@"vedioUrl":self.recentlyVedioFileUrl,@"photo":[self getImage:self.recentlyVedioFileUrl]};
...
-(void)mergeAndExportVideos:(NSMutableArray *)videosPathArray
    ...
    if (videosPathArray.count > 1) {
        [videosPathArray removeLastObject];
    }
    //创建音频通道容器
        ...
        BOOL ba = [self.audioTrack insertTimeRange:CMTimeRangeMake(kCMTimeZero, asset.duration)
                                           ofTrack:assetAudioTrack
                                            atTime:totalDuration
                                             error:&erroraudio];
        //[mixComposition insertTimeRange:CMTimeRangeMake(kCMTimeZero, asset.duration) ofAsset:asset atTime:totalDuration error:&errorVideo];
