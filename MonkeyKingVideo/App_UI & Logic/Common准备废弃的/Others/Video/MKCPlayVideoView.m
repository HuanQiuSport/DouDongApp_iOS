...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.mkPlayUserInfoView = [[MKPlayUserInfoView alloc] initWithFrame:CGRectMake(0,0, MAINSCREEN_WIDTH, MAINSCREEN_HEIGHT)];
...
//    NSLog(@"offset = %f",offset);
    if (_mkCurrentIndex == 0&& offset <= MAINSCREEN_HEIGHT) {
        
        [scrollView setContentOffset:CGPointMake(0, MAINSCREEN_HEIGHT) animated:NO];

        [WHToast showMessage:@"欧吼，已经到顶了"
                    duration:1
               finishHandler:nil];
        return;
    } else {
...
//        self.mkCurrentPlayer.view.frame = CGRectMake(0, MAINSCREEN_HEIGHT, MAINSCREEN_WIDTH, MAINSCREEN_HEIGHT);
