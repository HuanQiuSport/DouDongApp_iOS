...
- (void)mkLayOutView{
    ...
    [self.mkDescribeLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.bottom.equalTo(self.mas_bottom).offset(- kTabBarHeight - KBottomHeight- 5*KDeviceScale);
    ...
    [self.mkUserImageView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.left.equalTo(self.mas_left).offset(16 * KDeviceScale);
        
        make.bottom.equalTo(self.mkDescribeLabel.mas_top).offset(-5 * KDeviceScale);
        
        make.width.height.equalTo(@(35 * KDeviceScale));
    }];
    
    
    [self.mkUserNameLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        
        make.left.equalTo(self.mkUserImageView.mas_right).offset(5 * KDeviceScale);
        
        make.centerY.equalTo(self.mkUserImageView.mas_centerY);
        
        make.width.lessThanOrEqualTo(@(120 * KDeviceScale));
        
    }];
    
    [self.mkAttentionButton mas_makeConstraints:^(MASConstraintMaker *make) {
        
        make.left.equalTo(self.mkUserNameLabel.mas_right).offset(5 * KDeviceScale);
        
        make.centerY.equalTo(self.mkUserImageView.mas_centerY);
        
        make.width.equalTo(@(44 * KDeviceScale));
    ...
    [self.mkGouImageView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.width.equalTo(@(20 * KDeviceScale));
    ...
    [self.mkRightBtuttonView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.equalTo(@(180 * KDeviceScale ));
    ...
    [self.mkLoginView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.left.equalTo(self.mas_left).offset(5 * KDeviceScale);
        
        make.width.equalTo(self.mas_width).multipliedBy(0.5);
        
        make.height.equalTo(@(60 * KDeviceScale));
        
//        make.bottom.equalTo(self.mkUserImageView.mas_top).offset(-10 *KDeviceScale);
        make.top.equalTo(self.mas_top).offset(kNavigationBarHeight + kStatusBarHeight);
    }];
...
- (UIImageView *)mkUserImageView{
    ...
    if (!_mkUserImageView) {
        ...
        _mkUserImageView.layer.cornerRadius = 35*KDeviceScale/2;
...
//        [_mkAttentionButton setImage:[UIImage animatedImageNamed:@"勾选" duration:0.5] forState:UIControlStateSelected];
        ...
        _mkAttentionButton.layer.cornerRadius = 10 *KDeviceScale;
