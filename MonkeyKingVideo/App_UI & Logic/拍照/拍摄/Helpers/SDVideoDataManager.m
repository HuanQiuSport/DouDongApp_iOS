...
- (void)setCameraState:(VideoCameraState)cameraState {
     [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(changeVideoTime:) name:MKVideoTimeNotification object:nil];
...
- (void)changeVideoTime:(NSNotification *)timeNotification {
    
    NSString *timeString = [NSString stringWithFormat:@"%@",timeNotification.object];
    if ([timeString isEqualToString:@"拍摄1分钟"]) {
        _videoSecondTime = 60;
    } else if ([timeString isEqualToString:@"拍摄3分钟"]) {
        _videoSecondTime = 180;
    } else if ([timeString isEqualToString:@"拍摄5分钟"]) {
        _videoSecondTime = 300;
    }
...
- (void)dealloc {
    [[NSNotificationCenter defaultCenter] removeObserver:self name:MKVideoTimeNotification object:nil];
}
@end
